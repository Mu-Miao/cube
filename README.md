...
前端已经完成三套界面共存的 MVP 演示闭环，并在最近一轮补充了首屏性能、表单错误提示和周报展示优化：
...
- Element Plus、ECharts 和部分重组件改为按路由或按需加载；图片资源已压缩。
...
| 低 | 继续减包 | 按 ECharts 实际图表类型进一步裁剪模块 |

## 当前注意点

- 当前 UI 已经偏“炫酷仪表盘”风格，但仍保持操作台布局，不是营销落地页。
- ECharts 和 Element Plus 已拆分并延迟加载，但进入对应功能页时仍需下载相关 chunk。
- 管理员后台文件保留但入口正在开发中，不作为当前 MVP 演示范围。
- ESP32-S3 实机数据与控制主链路已经联调；后续重点是 WebSocket 断线重连和公网部署复核。
